...
class AppSharesController extends Controller
{
    ...
    private const THUMB_AUDIO_EXTENSIONS = ['mp3', 'm4a', 'm4b', 'aac', 'flac', 'ogg'];
    private const THUMB_EPUB_TYPES = ['application/epub+zip'];
    ...
    private function formatSharePrivate(Share $share): array
    {
        return [
            ...
            'files' => $share->files->map(function ($file) use ($share) {
                ...
                if ($this->canGenerateThumbnail($file->type, $file->name)) {
    ...
    public function fileThumbnail($shareId, $fileId)
    {
        ...
        if (file_exists($thumbPath)) {
            return $this->fileResponseWithCaching($thumbPath, 'image/webp');
        }

        // Need to generate thumbnail - get source file
        $sharePath = storage_path('app/shares/' . $share->path);
        $isVideo = in_array($file->type, self::THUMB_VIDEO_TYPES);
        ...
        if ($share->file_count == 1) {
            ...
            if ($isVideo) {
                $this->generateVideoThumbnail($sourcePath, $thumbPath);
            } elseif ($isAudio) {
                $this->generateAudioThumbnail($sourcePath, $thumbPath);
            } elseif ($isEpub) {
        ...
        try {
            if ($isVideo) {
                $this->generateVideoThumbnail($tempFile, $thumbPath);
            } elseif ($isAudio) {
                $this->generateAudioThumbnail($tempFile, $thumbPath);
            } elseif ($isEpub) {
     ...
     */
    private function generateImageThumbnail(string $sourcePath, string $outputPath): void
    {
    ...
    private function generateVideoThumbnail(string $videoPath, string $outputPath): void
    {
        ...
        $command = sprintf(
            'ffmpeg -ss 00:00:01 -i %s -vframes 1 -vf "scale=200:-1" -y %s 2>/dev/null',
        ...
        if ($returnCode !== 0 || !file_exists($tempPath)) {
            $command = sprintf(
                'ffmpeg -i %s -vframes 1 -vf "scale=200:-1" -y %s 2>/dev/null',
                escapeshellarg($videoPath),
                escapeshellarg($tempPath)
            );
            exec($command, $output, $returnCode);
     ...
     */
    private function generateAudioThumbnail(string $audioPath, string $outputPath): void
    {
        $tempPath = $outputPath . '.tmp.png';
        ...
        if ($returnCode !== 0 || !file_exists($tempPath)) {
            $command = sprintf(
                'ffmpeg -i %s -an -vf "scale=200:-1" -y %s 2>/dev/null',
                escapeshellarg($audioPath),
                escapeshellarg($tempPath)
            );
            exec($command, $output, $returnCode);
     ...
     */
    private function generateEpubThumbnail(string $epubPath, string $outputPath): void
    {
        $zip = new \ZipArchive();
     ...
     */
    private function findEpubCoverFromOpf(\ZipArchive $zip): ?string
    {
     ...
     */
    private function generatePdfThumbnail(string $pdfPath, string $outputPath): void
    {
        $tempPath = $outputPath . '.tmp.png';
        ...
        $command = sprintf(
            'pdftoppm -png -singlefile -f 1 -l 1 -r 72 %s %s 2>&1',
        ...
        if ($returnCode === 0 && file_exists($pdftoppmOutput)) {
            ...
            $command = sprintf(
                'pdftoppm -png -f 1 -l 1 -r 72 %s %s 2>&1',
                escapeshellarg($pdfPath),
                escapeshellarg($outputPath . '.tmp')
            );
            exec($command, $output, $returnCode);
     ...
     * Check if a file type supports thumbnail generation
     */
    private function canGenerateThumbnail(?string $mimeType, ?string $filename = null): bool
    {
        if (!$mimeType) {
            return false;
        }
        
        if (in_array($mimeType, self::THUMB_IMAGE_TYPES) || in_array($mimeType, self::THUMB_VIDEO_TYPES)) {
            return true;
        }
     ...
     */
    private function isAudioFile(?string $mimeType, ?string $filename = null): bool
    {
        if (in_array($mimeType, self::THUMB_AUDIO_TYPES)) {
            return true;
        }
     ...
     */
    private function isEpubFile(?string $mimeType, ?string $filename = null): bool
    {
        if (in_array($mimeType, self::THUMB_EPUB_TYPES)) {
            return true;
        }
     ...
     */
    private function isPdfFile(?string $mimeType, ?string $filename = null): bool
    {
        if (in_array($mimeType, self::THUMB_PDF_TYPES)) {
            return true;
        }
